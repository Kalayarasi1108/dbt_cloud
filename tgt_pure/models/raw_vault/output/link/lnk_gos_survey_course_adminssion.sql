INSERT INTO DATA_VAULT.CORE.LNK_GOS_SURVEY_COURSE_ADMISSION(LNK_GOS_SURVEY_COURSE_ADMISSION_KEY, HUB_GOS_SURVEY_KEY,
                                                       HUB_COURSE_ADMISSION_KEY, GOSID, COURSE_STU_ID, COURSE_SPK_CD,
                                                       COURSE_SPK_VER_NO, COURSE_AVAIL_YR, COURSE_LOCATION_CD,
                                                       COURSE_SPRD_CD, COURSE_AVAIL_KEY_NO, COURSE_SSP_NO, REPAR_COURSE,
                                                       REPAR_SSP_NO, REPAR_COURSE_CODE, SOURCE, LOAD_DTS, ETL_JOB_ID)
    WITH SPK_CAT_CD_CS AS (
       ...
       , EXIST_E307 AS (
        SELECT STU_ID, GOS.E307
        FROM ALL_E307
                 JOIN ODS.QILT.GOS_MQ_PIVOT GOS
                      ON GOS.E313 = ALL_E307.STU_ID AND GOS.E307 = ALL_E307.E307
        GROUP BY STU_ID, GOS.E307
    )
       ...
       , REPARENT_E307 AS (
        ...
        WHERE C_REP.PARENT_SSP_NO not in
              (SELECT ALL_E307.REPAR_SSP_NO FROM ALL_E307 WHERE ALL_E307.STU_ID = C_E307.STU_ID)
    )
...
--                SES_E307,
    ...
    SELECT MD5(
        ...
        )                                            LNK_GOS_SURVEY_COURSE_ADMISSION_KEY
         , MD5(IFNULL(GOS_MQ.GOSID, ''))             HUB_GOS_SURVEY_KEY
         , MD5(
                IFNULL(COURSE_E307.STU_ID, '') || ',' ||
                IFNULL(COURSE_E307.COURSE_CODE, '') || ',' ||
                IFNULL(COURSE_E307.SPK_VER_NO, 0) || ',' ||
    ...
    FROM ODS.QILT.GOS_MQ_PIVOT GOS_MQ
             JOIN COURSE_E307 ON COURSE_E307.E307 = GOS_MQ.E307
        AND COURSE_E307.STU_ID = GOS_MQ.E313
    WHERE RN = 1
      AND NOT EXISTS(
            SELECT NULL
            FROM DATA_VAULT.CORE.LNK_GOS_SURVEY_COURSE_ADMISSION L
            WHERE L.LNK_GOS_SURVEY_COURSE_ADMISSION_KEY = MD5(
                        IFNULL(GOS_MQ.GOSID, '') || ',' ||
                        IFNULL(COURSE_E307.STU_ID, '') || ',' ||
                        IFNULL(COURSE_E307.E307, '') || ',' ||
                        IFNULL(COURSE_E307.COURSE_CODE, '') || ',' ||
