INSERT INTO DATA_VAULT.CORE.LNK_STUDENT_UNIT_ENROLMENT_TO_UNIT_ENROLMENT_EP (LNK_STUDENT_UNIT_ENROLMENT_TO_UNIT_ENROLMENT_EP_KEY,
                                                                             HUB_UNIT_ENROLMENT_KEY,
                                                                             HUB_UNIT_ENROLMENT_EP_KEY, STU_ID,
                                                                             UNIT_SPK_CD, UNIT_SPK_VER_NO,
                                                                             UNIT_AVAIL_YR, UNIT_LOCATION_CD,
                                                                             UNIT_SPRD_CD, UNIT_AVAIL_KEY_NO,
                                                                             UNIT_SSP_NO, UNIT_PARENT_SSP_NO, EP_YEAR,
                                                                             EP_NO, SOURCE, LOAD_DTS, ETL_JOB_ID)
SELECT MD5(IFNULL(UN_SSP.STU_ID, '') || ',' ||
           ...
           )                                     LNK_STUDENT_UNIT_ENROLMENT_TO_UNIT_ENROLMENT_EP_KEY,
...
WHERE UN_SSP.SSP_NO <> UN_SSP.PARENT_SSP_NO
  AND NOT EXISTS(
        SELECT NULL
        FROM DATA_VAULT.CORE.LNK_STUDENT_UNIT_ENROLMENT_TO_UNIT_ENROLMENT_EP L
        WHERE L.HUB_UNIT_ENROLMENT_KEY = MD5(IFNULL(UN_SSP.STU_ID, '') || ',' ||
          ...
          AND L.HUB_UNIT_ENROLMENT_EP_KEY = MD5(IFNULL(UN_SSP.STU_ID, '') || ',' ||
                                                IFNULL(UN_SPK_DET.SPK_CD, '') || ',' ||
                                                IFNULL(UN_SPK_DET.SPK_VER_NO, 0) || ',' ||
                                                IFNULL(UN_SSP.AVAIL_YR, 0) || ',' ||
                                                IFNULL(UN_SSP.LOCATION_CD, '') || ',' ||
