INSERT INTO DATA_VAULT.CORE.LNK_STUDENT_COURSE_ADMISSION (LNK_STU_COURSE_ADMISSION_KEY, HUB_STUDENT_KEY,
                                                          HUB_COURSE_ADMISSION_KEY, STU_ID, SPK_CD, SPK_VER_NO,
                                                          AVAIL_YR, LOCATION_CD, SPRD_CD, AVAIL_KEY_NO, SSP_NO, SOURCE,
                                                          LOAD_DTS, ETL_JOB_ID)
SELECT MD5(IFNULL(CS_SSP.STU_ID, '') || ',' ||
           ...
           ) LNK_STU_COURSE_ADMISSION_KEY,
       MD5(CS_SSP.STU_ID)                               HUB_STUDENT_KEY,
...
WHERE CS_SSP.SSP_NO = PARENT_SSP_NO
  AND NOT EXISTS(
        SELECT NULL
        FROM DATA_VAULT.CORE.LNK_STUDENT_COURSE_ADMISSION L
      ...
      AND L.HUB_STUDENT_KEY = MD5(CS_SSP.STU_ID)
    );
