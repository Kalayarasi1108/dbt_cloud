...
SELECT DATA_VAULT.CORE.SEQ.NEXTVAL               SAT_UNIT_ENROLMENT_EP_SK,
       ...
       'Y'                                       IS_DELETED
FROM (
         SELECT HUB_UNIT_ENROLMENT_EP_KEY,
                HASH_MD5,
                LOAD_DTS,
                IS_DELETED,
                LEAD(LOAD_DTS) OVER (PARTITION BY HUB_UNIT_ENROLMENT_EP_KEY ORDER BY LOAD_DTS ASC) EFFECTIVE_END_DTS
         FROM DATA_VAULT.CORE.SAT_UNIT_ENROLMENT_EP
     ) S
