...
SELECT DATA_VAULT.CORE.SEQ.NEXTVAL               SAT_UNIT_ENROLMENT_SANCTION_SK,
       MD5(
...
FROM ODS.AMIS.S1SSP_SCT_DTL SSP_SCT_DTL
         ...
         JOIN ODS.AMIS.S1SPK_DET SPK_DET
              ON STU_SPK.SPK_NO = SPK_DET.SPK_NO
                  and STU_SPK.SPK_VER_NO = SPK_DET.SPK_VER_NO AND SPK_DET.SPK_CAT_CD = 'UN'
...
WHERE NOT EXISTS(
        ...
        FROM (SELECT HUB_UNIT_ENROLMENT_SANCTION_KEY,
              ...
              FROM DATA_VAULT.CORE.SAT_UNIT_ENROLMENT_SANCTION) S
        WHERE S.EFFECTIVE_END_DTS IS NULL
          AND S.HUB_UNIT_ENROLMENT_SANCTION_KEY = MD5(
                    IFNULL(SSP_SCT_DTL.SSP_NO, 0) || ',' ||
                    IFNULL(SSP_SCT_DTL.SEQ_NO, 0) || ',' ||
                    IFNULL(SSP_SCT_DTL.STU_ID, '')
            )
